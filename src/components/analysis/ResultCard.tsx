...
const CircularProgress = ({ value, maxValue, status, displayValue, unit }: CircularProgressProps) => {
  ...
  const getBgColor = () => {
    ...
    if (status === "baixo") return "rgba(128, 0, 32, 0.15)";
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="absolute flex flex-col items-center justify-center" style={{ width: size, height: size }}>
        <span className={`text-xl font-bold ${status === "normal" ? "text-green-600 dark:text-green-400" : status === "baixo" ? "text-[#800020]" : "text-destructive"}`}>
          {displayValue}
        </span>
...
export const ResultCard = ({ item }: ResultCardProps) => {
  const getStatusBadge = () => {
    switch (item.status) {
      ...
      case "baixo":
        return <Badge className="bg-[#800020] hover:bg-[#800020]/90 text-white">Baixo</Badge>;
      default:
        return null;
    }
  };
